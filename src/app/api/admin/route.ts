...
export async function GET() {
  ...
  try {
    const [profilesRes, scansRes, featureRes] = await Promise.all([
      fetch(`${url}/rest/v1/profiles?select=plan`, { headers }),
      fetch(`${url}/rest/v1/scans?select=id,url,status,provider,created_at&order=created_at.desc&limit=10`, { headers }),
      fetch(`${url}/rest/v1/feature_requests?select=id,name,email,message,created_at&order=created_at.desc&limit=10`, { headers }),
    ]);
